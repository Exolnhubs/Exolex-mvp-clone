...
export default function RequestsPage() {
  ...
  const loadRequests = async () => {
    ...
    const { data, error } = await supabase
      ...
      .eq('is_accepted', false).is('assigned_lawyer_id', null)
    ...
    // حساب الإحصائيات
    const readyCount = requestsList.filter(r => r.base_price && r.base_price > 0).length
    const needsQuoteCount = requestsList.filter(r => !r.base_price || r.base_price === 0).length
  ...
  const getFilteredData = () => {
    ...
    if (activeTab === 'ready_to_accept') {
      data = requests.filter(r => r.base_price && r.base_price > 0)
                    ...
                    <div className="flex items-start justify-between mb-3">
                      <div>
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          {activeTab === 'ready_to_accept' && request.base_price && (
                            <span className="px-2 py-0.5 text-xs bg-green-100 text-green-700 rounded-full font-bold">
                              {request.base_price.toLocaleString('ar-SA')} ر.س
                            </span>
                          )}
                        </div>
