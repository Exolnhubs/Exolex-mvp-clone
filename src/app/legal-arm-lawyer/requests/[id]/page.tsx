...
// ═══════════════════════════════════════════════════════════════════════════════
// 📋 تفاصيل الطلب - محامي الذراع القانوني
// تاريخ: 1 يناير 2026
// ملاحظة: محامي الذراع القانوني يعمل بمفرده - لا يوجد مشاركة مع آخرين
// ═══════════════════════════════════════════════════════════════════════════════

const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  pending: { label: 'قيد الانتظار', color: 'bg-gray-100 text-gray-700', icon: Clock },
  assigned: { label: 'معين', color: 'bg-amber-100 text-amber-700', icon: User },
  accepted: { label: 'مقبول', color: 'bg-blue-100 text-blue-700', icon: CheckCircle },
  in_progress: { label: 'قيد العمل', color: 'bg-amber-100 text-amber-700', icon: Timer },
...
const typeConfig: Record<string, { label: string; color: string }> = {
  consultation: { label: 'استشارة', color: 'bg-amber-500' },
...
export default function ArmLawyerRequestDetailsPage() {
  ...
  // حساب الصلاحيات - محامي الذراع القانوني هو دائماً صاحب الطلب
  ...
  const getSlaStatus = () => {
    ...
    if (request.sla_deadline) {
      ...
      return { text: `${hours} ساعة`, color: 'bg-amber-100 text-amber-700' }
    }
    return null
  }
  ...
  if (!request) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={() => router.back()} className="mt-4 px-4 py-2 bg-amber-600 text-white rounded-lg">
          رجوع
        </button>
      </div>
    )
              ...
              {activeTab === 'overview' && (
                <div className="space-y-6">
                  {request.nolex_guidance && (
                    <div className="p-4 bg-gradient-to-r from-amber-50 to-blue-50 rounded-xl border border-amber-200">
                      <h4 className="font-bold text-amber-800 mb-2 flex items-center gap-2">
                        <Bot className="w-5 h-5" />
                        توجيه NOLEX
                      </h4>
                      <p className="text-amber-700">{request.nolex_guidance}</p>
                    </div>
                  )}
                    ...
                    {messages.length > 0 ? messages.map(msg => (
                      ...
                      >
                        <div className={`max-w-[70%] p-3 rounded-lg ${
                          ...
                          <p>{msg.content}</p>
                          <p className={`text-xs mt-1 ${msg.sender_type === 'lawyer' ? 'text-amber-100' : 'text-slate-400'}`}>
                      ...
                      <div className="space-y-4">
                        {timeline.map(item => (
                          <div key={item.id} className="flex gap-4 relative">
                            <div className="w-8 h-8 rounded-full bg-amber-500 flex items-center justify-center z-10">
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-2">
              <button className="w-full flex items-center gap-3 px-3 py-2 text-right hover:bg-amber-50 rounded-lg text-amber-600 transition-colors">
                <Bot className="w-5 h-5" />
          ...
          <div className="bg-white rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-2">
              ...
              {['accepted', 'in_progress'].includes(request.status) && canCloseRequest && (
                <button 
                  onClick={handleClose} 
                  className="w-full flex items-center gap-3 px-3 py-2 bg-amber-50 hover:bg-amber-100 rounded-lg text-amber-700 transition-colors"
                >
          ...
          {timeLogs.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm p-4">
              ...
              <div className="mt-3 pt-3 border-t flex justify-between">
                ...
                <span className="font-bold text-amber-600">{totalHours} ساعة</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleAccept} className="flex-1 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700">قبول</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-amber-500"
                >
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleTimeLog} className="flex-1 px-4 py-2 bg-amber-600 text-white rounded-lg hover:bg-amber-700">حفظ</button>
            </div>
          </div>
        </div>
      )}
