  ...
  if (!caseData) {
    return (
      <div className="text-center py-12">
        ...
        <Link href="/partner/cases" className="text-emerald-600 hover:underline mt-4 inline-block">العودة للقضايا</Link>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <button onClick={() => setShowStatusModal(true)} className="px-4 py-2 bg-emerald-600 hover:bg-blue-700 text-white rounded-lg flex items-center gap-2">
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm">
          ...
          <div className="space-y-4">
            <div className="p-3 bg-blue-50 rounded-lg">
              <p className="text-xs text-emerald-600 mb-1">المدعي</p>
      ...
      <div className="bg-white rounded-xl shadow-sm">
        <div className="flex border-b overflow-x-auto">
          {[
            ...
            <button key={tab.key} onClick={() => setActiveTab(tab.key as any)} className={`flex items-center gap-2 px-4 py-3 font-medium whitespace-nowrap border-b-2 transition-all ${activeTab === tab.key ? 'text-emerald-600 border-emerald-600' : 'text-slate-600 border-transparent hover:text-slate-800'}`}>
          ...
          {activeTab === 'sessions' && (
            <div className="space-y-4">
              <div className="flex justify-end">
                <button onClick={() => setShowSessionModal(true)} className="flex items-center gap-2 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-blue-700"><Plus className="w-4 h-4" />إضافة جلسة</button>
              </div>
              {sessions.length > 0 ? (
                <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {documents.map(doc => (
                    <div key={doc.id} className="p-4 bg-slate-50 rounded-xl flex items-center gap-4">
                      <div className="w-10 h-10 rounded-lg bg-blue-100 flex items-center justify-center"><FileText className="w-5 h-5 text-emerald-600" /></div>
                  ...
                  <div className="space-y-4">
                    {timeline.map(item => (
                      <div key={item.id} className="flex gap-4 relative">
                        <div className="w-8 h-8 rounded-full bg-emerald-500 flex items-center justify-center z-10"><CheckCircle className="w-4 h-4 text-white" /></div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-4"><button onClick={() => setShowSessionModal(false)} className="flex-1 px-4 py-2 border border-slate-300 text-slate-700 rounded-lg">إلغاء</button><button onClick={addSession} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg">إضافة</button></div>
            </div>
          </div>
        </div>
      )}
          ...
          <div className="bg-white rounded-2xl w-full max-w-md">
            ...
            <div className="p-6 space-y-2 max-h-[60vh] overflow-y-auto">
              {Object.entries(statusConfig).map(([key, val]) => (
                <button key={key} onClick={() => updateCaseStatus(key)} className={`w-full p-3 text-right rounded-lg border transition-all flex items-center justify-between ${caseData.court_status === key ? 'border-emerald-500 bg-blue-50' : 'border-slate-200 hover:border-blue-300'}`}>
                  <span className={`px-2 py-1 rounded-full text-xs font-medium ${val.color}`}>{val.label}</span>
                  {caseData.court_status === key && <CheckCircle className="w-5 h-5 text-emerald-600" />}
                </button>
              ))}
            </div>
