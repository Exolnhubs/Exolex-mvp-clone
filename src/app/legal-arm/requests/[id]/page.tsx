...
// ═══════════════════════════════════════════════════════════════════════════════
// 📋 تفاصيل الطلب - الذراع القانوني
// تاريخ: 1 يناير 2026
// تم تصحيح أسماء الجداول والحقول
// ═══════════════════════════════════════════════════════════════════════════════

const statusConfig: Record<string, { label: string; color: string; icon: any }> = {
  pending: { label: 'قيد الانتظار', color: 'bg-gray-100 text-gray-700', icon: Clock },
  assigned: { label: 'معين', color: 'bg-emerald-100 text-emerald-700', icon: User },
  accepted: { label: 'مقبول', color: 'bg-teal-100 text-teal-700', icon: CheckCircle },
  in_progress: { label: 'قيد العمل', color: 'bg-green-100 text-green-700', icon: Timer },
...
const typeConfig: Record<string, { label: string; color: string }> = {
  consultation: { label: 'استشارة', color: 'bg-emerald-500' },
  case: { label: 'قضية', color: 'bg-teal-500' },
  extra_service: { label: 'خدمة إضافية', color: 'bg-green-500' },
}
...
export default function LegalArmRequestDetailsPage() {
  ...
  const canAddComment = isOwner || isCollaborator || isManager
  ...
  const loadData = async () => {
    try {
      ...
      // جلب معلومات المحامي الحالي من جدول lawyers
      const { data: lawyerData, error: lawyerError } = await supabase
        .from('lawyers')
        .select('id, user_id, lawyer_type, legal_arm_id, license_number, license_expiry, full_name, role')
        .eq('id', lawyerId)
        .single()
      ...
      setCurrentUser({
        ...
        role: lawyerData.role || 'lawyer',
        isManager: ['manager', 'senior_lawyer', 'admin'].includes(lawyerData.role || ''),
        hasLicense: !!lawyerData.license_number,
        licenseValid,
        legalArmId: lawyerData.legal_arm_id || ''
      })
      ...
      // جلب اسم المحامي المعين
      if (requestData?.assigned_lawyer_id) {
        const { data: assignedLawyer } = await supabase
          .from('lawyers')
          .select('id, full_name, email, phone')
          .eq('id', requestData.assigned_lawyer_id)
          .single()
        requestData.assigned_lawyer = assignedLawyer
      }
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      router.push('/legal-arm/requests')
    } catch (error) {
      toast.error('حدث خطأ')
    }
  }
  ...
  const handleTransferRequest = async () => {
    if (!selectedLawyer || !transferReason.trim()) {
    ...
    try {
      await supabase.from('request_transfers').insert({
        ...
        to_lawyer_id: selectedLawyer,
        transfer_reason: transferReason,
        transferred_by: currentUser?.id,
        status: 'pending'
      })
      
      await logActivity('transfer_request', 'طلب تحويل الطلب', { to: selectedLawyer })
      
      // إشعار المدراء
      const managers = lawyers.filter(l => ['manager', 'senior_lawyer'].includes(l.role || ''))
  ...
  const handleTransferToSelf = async () => {
    ...
    try {
      const previousOwner = request.assigned_lawyer_id

      await supabase
        .from('service_requests')
        .update({ 
          assigned_lawyer_id: currentUser?.id,
  ...
  const handleCollaboratorRequest = async () => {
    ...
    try {
      await supabase.from('request_collaborators').insert({
        ...
        lawyer_name: selectedLawyerData?.full_name,
      ...
      await logActivity('add_collaborator', `إضافة مشارك: ${selectedLawyerData?.full_name}`)
      await sendNotification(selectedLawyer, 'تمت إضافتك كمشارك', `أضافك ${currentUser?.name} في طلب`, 'collaborator')
  ...
  const handleSendInternalMessage = async () => {
    ...
    try {
      ...
      while ((match = mentionRegex.exec(newInternalMessage)) !== null) {
        const mentionedLawyer = lawyers.find(l => l.full_name?.includes(match[1]))
        if (mentionedLawyer) mentions.push(mentionedLawyer.id)
      }
  ...
  const getSlaStatus = () => {
    ...
    if (request.sla_deadline) {
      ...
      return { text: `${hours} ساعة`, color: 'bg-emerald-100 text-emerald-700' }
    }
    return null
  }
  ...
  const insertMention = (lawyer: any) => {
    const words = newInternalMessage.split(' ')
    words[words.length - 1] = `@${lawyer.full_name} `
  ...
  if (!request) {
    return (
      <div className="text-center py-12">
        ...
        <button onClick={() => router.back()} className="mt-4 px-4 py-2 bg-emerald-600 text-white rounded-lg">
          رجوع
        </button>
      </div>
    )
      ...
      <div className="bg-white rounded-xl shadow-sm p-4">
        <div className="flex items-center justify-between flex-wrap gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            {isOwner && (
              <span className="px-2 py-1 bg-teal-100 text-teal-700 rounded-full text-xs font-medium flex items-center gap-1">
                ...
                <div className="space-y-6">
                  {request.nolex_guidance && (
                    <div className="p-4 bg-gradient-to-r from-emerald-50 to-teal-50 rounded-xl border border-emerald-200">
                      ...
                      <p className="text-emerald-700">{request.nolex_guidance}</p>
                    </div>
                  )}
                    ...
                    {messages.length > 0 ? messages.map(msg => (
                      ...
                      >
                        <div className={`max-w-[70%] p-3 rounded-lg ${
                          ...
                          <p>{msg.content}</p>
                          <p className={`text-xs mt-1 ${msg.sender_type === 'lawyer' ? 'text-emerald-100' : 'text-slate-400'}`}>
              ...
              {activeTab === 'team' && (
                <div className="space-y-4">
                  <div className="flex flex-wrap gap-2 pb-4 border-b">
                    ...
                    <span className="px-2 py-1 bg-teal-100 text-teal-700 rounded-full text-sm flex items-center gap-1">
                      <UserCheck className="w-3 h-3" />
                      {request.assigned_lawyer?.full_name || 'غير معين'}
                    </span>
                    ...
                    {internalChat.length > 0 ? internalChat.map(msg => (
                      <div key={msg.id} className="flex gap-3">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-sm font-bold ${
                          msg.sender_type === 'manager' ? 'bg-emerald-600' : 'bg-teal-500'
                        }`}>
                          {msg.sender_name?.[0] || '؟'}
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center gap-2">
                            ...
                            {msg.sender_type === 'manager' && (
                              <span className="text-xs bg-emerald-100 text-emerald-600 px-1 rounded">مدير</span>
                            )}
                        ...
                        <div className="flex-1 relative">
                          ...
                          {showMentionList && (
                            <div className="absolute bottom-full mb-1 right-0 w-64 bg-white border rounded-lg shadow-lg max-h-48 overflow-y-auto z-10">
                              {lawyers
                                .filter(l => l.full_name?.includes(mentionSearch))
                                .slice(0, 5)
                                .map(lawyer => (
                                  <button
                                    key={lawyer.id}
                                    onClick={() => insertMention(lawyer)}
        ...
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-3 text-sm">
              <div className="flex justify-between">
                ...
                <span className="text-slate-800 font-medium">{request.assigned_lawyer?.full_name || 'غير معين'}</span>
              </div>
              <div className="flex justify-between">
          ...
          <div className="bg-white rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-2">
              <button className="w-full flex items-center gap-3 px-3 py-2 text-right hover:bg-emerald-50 rounded-lg text-emerald-600 transition-colors">
                <Bot className="w-5 h-5" />
          ...
          <div className="bg-white rounded-xl shadow-sm p-4">
            ...
            <div className="space-y-2">
              ...
              {isManager && !isOwner && canTransferToSelf && ['assigned', 'accepted', 'in_progress'].includes(request.status) && (
                <button 
                  onClick={() => setShowTransferToSelfModal(true)}
                  className="w-full flex items-center gap-3 px-3 py-2 bg-emerald-50 hover:bg-emerald-100 rounded-lg text-emerald-700 transition-colors"
                >
              ...
              {['accepted', 'in_progress'].includes(request.status) && (
                <>
                  ...
                  {canCloseRequest && (
                    <button 
                      onClick={handleClose} 
                      className="w-full flex items-center gap-3 px-3 py-2 bg-teal-50 hover:bg-teal-100 rounded-lg text-teal-700 transition-colors"
                    >
              ...
              {['completed', 'closed'].includes(request.status) && (
                <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg text-emerald-700 text-center">
          ...
          {timeLogs.length > 0 && (
            <div className="bg-white rounded-xl shadow-sm p-4">
              ...
              <div className="mt-3 pt-3 border-t flex justify-between">
                ...
                <span className="font-bold text-emerald-600">{totalHours} ساعة</span>
              </div>
            </div>
          )}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleAccept} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">قبول</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <select
              ...
              className="w-full px-4 py-2 border border-slate-300 rounded-lg mb-4 bg-white focus:ring-2 focus:ring-emerald-500"
            >
              <option value="">اختر المحامي</option>
              {lawyers.filter(l => l.id !== currentUser?.id && l.license_number).map(lawyer => (
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleTransferRequest} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">إرسال الطلب</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-lg mb-4">
              ...
              <ul className="mt-2 text-emerald-600 text-sm list-disc list-inside">
            ...
            <p className="text-slate-600 mb-4">
              المحامي الحالي: <strong>{request.assigned_lawyer?.full_name || 'غير معين'}</strong>
            </p>
            <div className="flex gap-3">
              ...
              <button onClick={handleTransferToSelf} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">تحويل لي</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            >
              <option value="">اختر المحامي</option>
              {lawyers
                .filter(l => l.id !== currentUser?.id && !collaborators.find(c => c.lawyer_id === l.id))
                .map(lawyer => (
                  <option key={lawyer.id} value={lawyer.id}>{lawyer.full_name}</option>
                ))}
            </select>
            ...
            <div className="flex gap-3">
              ...
              <button onClick={handleCollaboratorRequest} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl w-full max-w-md p-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-4 py-2 border border-slate-300 rounded-lg bg-white focus:ring-2 focus:ring-emerald-500"
                >
            ...
            <div className="flex gap-3 mt-6">
              ...
              <button onClick={handleTimeLog} className="flex-1 px-4 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700">حفظ</button>
            </div>
          </div>
        </div>
      )}
