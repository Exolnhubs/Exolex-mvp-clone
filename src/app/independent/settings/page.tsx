          ...
          {activeTab === 'notifications' && (
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                {[
                  { key: 'notify_new_requests', label: 'طلبات جديدة', icon: '📋', desc: 'إشعار عند وجود طلب جديد في مجالك' },
                  { key: 'notify_fixed_price', label: 'طلبات مسعرة', icon: '💰', desc: 'طلبات بسعر محدد' },
                  { key: 'notify_quote_requests', label: 'طلبات عروض أسعار', icon: '📝', desc: 'طلبات تحتاج عرض سعر' },
                  { key: 'notify_offer_accepted', label: 'قبول العرض', icon: '✅', desc: 'عند قبول عرضك' },
                  { key: 'notify_offer_rejected', label: 'رفض العرض', icon: '❌', desc: 'عند رفض عرضك' },
                  ...
                  { key: 'notify_system_updates', label: 'تحديثات النظام', icon: '🔔', desc: 'إعلانات وتحديثات' },
                ].map(item => (
              ...
              <div className="pt-4 border-t">
                <div className="flex items-center justify-between mb-4">
                  <h3 className="font-bold text-slate-700">📂 مجالات الإشعارات</h3>
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={notifSettings.notify_all_categories}
                      onChange={(e) => setNotifSettings({ ...notifSettings, notify_all_categories: e.target.checked })}
                ...
                <p className="text-sm text-slate-500 mb-3">
                  مجالاتك: {lawyerCategories.length} | إضافية: {notifSettings.selected_categories?.length || 0}
                </p>

                <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                  {categories.map(cat => {
                    const isMyCategory = lawyerCategories.includes(cat.id)
                    const isSelected = notifSettings.selected_categories?.includes(cat.id)
                    const IconComponent = getIcon(cat.icon)
                    return (
                      <button
                        key={cat.id}
                        onClick={() => !isMyCategory && toggleCategory(cat.id)}
                        disabled={notifSettings.notify_all_categories}
                        className={`p-3 rounded-lg text-sm text-right transition-all flex flex-col items-start gap-2 ${
                          isMyCategory
                            ? 'bg-amber-100 text-amber-700 border-2 border-amber-400'
                            : isSelected
                            ? 'bg-blue-100 text-blue-700 border-2 border-blue-400'
                            : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                        } ${notifSettings.notify_all_categories ? 'opacity-50' : ''}`}
                      >
                        <IconComponent className="w-6 h-6" />
                        <span>{cat.name_ar}</span>
                        {isMyCategory && <span className="text-xs opacity-70">(مجالك)</span>}
                      </button>
                    )
                  })}
                </div>
