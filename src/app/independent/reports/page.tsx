...
export default function ReportsPage() {
  ...
  const [activeReport, setActiveReport] = useState<'earnings' | 'requests' | 'cases' | 'ratings' | 'performance'>('earnings')
  const [period, setPeriod] = useState<'month' | '3months' | 'year' | 'all'>('month')
  const [isExporting, setIsExporting] = useState(false)

  // البيانات
  const [earnings, setEarnings] = useState<any>({ total: 0, byMonth: [], byType: [] })
  const [requests, setRequests] = useState<any>({ total: 0, byStatus: [], byType: [], byMonth: [] })
  ...
  const loadData = async () => {
    try {
      ...
      const { data: offersData } = await supabase
        .from('service_offers')
        .select('id, price, lawyer_commission_amount, created_at, request:request_id(request_type, status)')
        .or(`lawyer_id.eq.${lawyerId},created_by.eq.${lawyerId}`)
        .eq('status', 'accepted')
        .gte('created_at', start.toISOString())

      const earningsTotal = (offersData || []).reduce((sum, o) => sum + (o.lawyer_commission_amount || o.price * 0.7 || 0), 0)
      
      // حسب الشهر
      const earningsByMonth: Record<string, number> = {}
      ;(offersData || []).forEach(o => {
        const month = new Date(o.created_at).toLocaleString('ar-SA', { month: 'short', year: 'numeric' })
        earningsByMonth[month] = (earningsByMonth[month] || 0) + (o.lawyer_commission_amount || o.price * 0.7 || 0)
      })

      // حسب النوع
      const earningsByType: Record<string, number> = {}
      ;(offersData || []).forEach(o => {
        const type = o.request?.request_type || 'other'
        earningsByType[type] = (earningsByType[type] || 0) + (o.lawyer_commission_amount || o.price * 0.7 || 0)
      })

      setEarnings({
        total: earningsTotal,
        byMonth: Object.entries(earningsByMonth).map(([month, amount]) => ({ month, amount })),
        byType: Object.entries(earningsByType).map(([type, amount]) => ({ type, amount }))
      })
      ...
      const { data: requestsData } = await supabase
        ...
        .select('id, status, request_type, created_at, completed_at')
      ...
      const { data: lawyerData } = await supabase
        .from('lawyers')
        .select('sla_compliance_rate, total_requests_completed, total_requests_handled')
        .eq('id', lawyerId)
        .single()
      ...
      setPerformance({
        sla: lawyerData?.sla_compliance_rate || 0,
        avgResponse: Math.round(avgResponseTime * 10) / 10,
        completion: lawyerData?.total_requests_handled 
          ? Math.round((lawyerData.total_requests_completed / lawyerData.total_requests_handled) * 100) 
          : 0
      })
  ...
  const exportExcel = async () => {
    ...
    try {
      ...
      if (activeReport === 'earnings') {
        csvContent = 'الشهر,المبلغ\n'
        earnings.byMonth.forEach((row: any) => {
  ...
  const generateReportHTML = () => {
    ...
    return `
      ...
      <head>
        <meta charset="UTF-8">
        <title>تقرير ${activeReport === 'earnings' ? 'الأرباح' : activeReport === 'requests' ? 'الطلبات' : activeReport === 'cases' ? 'القضايا' : activeReport === 'ratings' ? 'التقييمات' : 'الأداء'}</title>
        <style>
      ...
      <body>
        <h1>📊 تقرير ${activeReport === 'earnings' ? 'الأرباح' : activeReport === 'requests' ? 'الطلبات' : activeReport === 'cases' ? 'القضايا' : activeReport === 'ratings' ? 'التقييمات' : 'الأداء'}</h1>
        ...
        ${activeReport === 'earnings' ? `
          <div class="stat-box">
            ...
            <div class="total">${earnings.total.toLocaleString()} ر.س</div>
          </div>
          <h2>الأرباح حسب الشهر</h2>
          <table>
            <tr><th>الشهر</th><th>المبلغ</th></tr>
            ${earnings.byMonth.map((r: any) => `<tr><td>${r.month}</td><td>${r.amount.toLocaleString()} ر.س</td></tr>`).join('')}
          </table>
        ` : ''}
        ...
        ${activeReport === 'requests' ? `
          ...
          <table>
            ...
            ${requests.byStatus.map((r: any) => `<tr><td>${r.status}</td><td>${r.count}</td></tr>`).join('')}
          </table>
        ` : ''}
        ...
        ${activeReport === 'cases' ? `
          ...
          <table>
            <tr><th>الحالة</th><th>العدد</th></tr>
            ${cases.byStatus.map((r: any) => `<tr><td>${r.status}</td><td>${r.count}</td></tr>`).join('')}
          </table>
        ` : ''}
        ...
        <div className="bg-white rounded-xl shadow-sm p-4">
          <div className="flex flex-wrap items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 overflow-x-auto">
              {[
                { key: 'earnings', label: '💰 الأرباح', color: 'emerald' },
            ...
            <>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  <p className="text-slate-500 text-sm">عدد الصفقات</p>
                  <p className="text-3xl font-bold text-slate-800 mt-2">{earnings.byMonth.length || 0}</p>
                </div>
                <div className="bg-white rounded-xl p-6 shadow-sm">
                  <p className="text-slate-500 text-sm">متوسط الصفقة</p>
                  <p className="text-3xl font-bold text-slate-800 mt-2">
                    {earnings.byMonth.length > 0 
                      ? Math.round(earnings.total / earnings.byMonth.length).toLocaleString() 
                      : 0}
                  </p>
                </div>
              </div>

              {/* رسم بياني - الأرباح حسب الشهر */}
              <div className="bg-white rounded-xl shadow-sm p-6">
                <h3 className="font-bold text-slate-800 mb-4">📈 الأرباح حسب الشهر</h3>
                {earnings.byMonth.length > 0 ? (
                  <div className="space-y-3">
                    {earnings.byMonth.map((row: any, i: number) => (
                      <div key={i} className="flex items-center gap-4">
                        <span className="w-24 text-sm text-slate-600">{row.month}</span>
                        <div className="flex-1 h-8 bg-slate-100 rounded-full overflow-hidden">
                          <div
                            className="h-full bg-gradient-to-r from-emerald-400 to-emerald-500 rounded-full flex items-center justify-end px-3"
                            style={{ width: `${(row.amount / getMaxValue(earnings.byMonth, 'amount')) * 100}%` }}
                          >
                            <span className="text-xs text-white font-medium">{row.amount.toLocaleString()}</span>
                          </div>
                        </div>
                      </div>
                    ))}
              ...
              {/* الأرباح حسب النوع */}
              <div className="bg-white rounded-xl shadow-sm p-6">
                <h3 className="font-bold text-slate-800 mb-4">📊 الأرباح حسب نوع الخدمة</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {earnings.byType.map((row: any, i: number) => (
                    <div key={i} className="bg-slate-50 rounded-lg p-4 text-center">
                      <p className="text-sm text-slate-500">{translateType(row.type)}</p>
                      <p className="text-xl font-bold text-emerald-600 mt-1">{row.amount.toLocaleString()}</p>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeReport === 'requests' && (
            <>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                {requests.byStatus.slice(0, 3).map((row: any, i: number) => (
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl p-6 border border-blue-200">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
                  <div className="flex items-start gap-2">
                    <span>⏰</span>
                    <p className="text-blue-700">استجب للطلبات الجديدة خلال 24 ساعة</p>
                  </div>
                  <div className="flex items-start gap-2">
                    <span>📋</span>
                    <p className="text-blue-700">أكمل الطلبات ضمن المدة المحددة</p>
                  </div>
                  <div className="flex items-start gap-2">
                    <span>💬</span>
                    <p className="text-blue-700">حافظ على تواصل مستمر مع العملاء</p>
                  </div>
                </div>
              </div>
            </>
